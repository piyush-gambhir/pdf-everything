...
import { addPageNumbers } from '../../../workers/pdf-core-worker.client.js';
import { PageNumberOptionsSchema } from '@pdf-everything/types';
...
export class PageNumbersController {
  ...
  @ApiOperation({ summary: 'Stamp page numbers onto PDF pages' })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        options: {
          type: 'string',
          description: 'JSON: { format, position, fontSize, margin, startNumber, pages? }',
        },
      },
    },
  })
  async run(
    ...
    const opts = parseOptions(body.options, PageNumberOptionsSchema);
    const out = await addPageNumbers(buffers[0]!, opts);
    await respondWithPdf({
      res,
      buffer: out,
      filename: outputName(originalNames[0], 'numbered'),
      outputMode: output,
      files: this.files,
    });
  }
